...
path_anndata ="/nfs/team361/am74/mintflow/revision/skin_perturbation/per_section_raw/"
...
for filename in tqdm(files, desc="Processing .h5ad files"):
    ...
    adata.obs["TissueSectionID_for_MintFlow"] = adata.obs["sample_id__xenium_id_recoded"].astype("category")

    # MintFlow’s required biological batch = your "donor"
    adata.obs["batchID_for_MintFlow"] = adata.obs["sample_id__xenium_id_recoded"].astype("category")
...
for idx, filename in enumerate(files, start=1):
    ...
    config_data_train["list_tissue"][key]["obskey_cell_type"] = "lvl5_annotation"
    # column containing cell type labels

    config_data_train["list_tissue"][key]["obskey_sliceid_to_checkUnique"] = "sample_id__xenium_id_recoded"
    ...
    config_data_train["list_tissue"][key]["obskey_biological_batch_key"] = "sample_id__xenium_id_recoded"
    # batch ID
...
for idx, filename in enumerate(files, start=1):
    ...
    # Column containing cell type labels
    config_data_evaluation["list_tissue"][key]["obskey_cell_type"] = "lvl5_annotation"

    # unique tissue/slice ID
    config_data_evaluation["list_tissue"][key]["obskey_sliceid_to_checkUnique"] = "sample_id__xenium_id_recoded"
    ...
    # batch identifier
    config_data_evaluation["list_tissue"][key]["obskey_biological_batch_key"] = "sample_id__xenium_id_recoded"
...
path_output_files = "/nfs/team361/am74/mintflow/revision/skin_perturbation/mintflow_runs/run1_tiger/"
# TODO:MODIFY: the path where checkpoints and other files are saved during training.

with open("/nfs/team361/am74/mintflow/revision/skin_perturbation/mintflow_runs/run1_tiger/run1data_mintflow.pkl", "wb") as f:  # wb = write binary
    pickle.dump(data_mintflow, f)
...
for index_epoch in tqdm(range(config_training['num_training_epochs']), desc='Training epoch'):
    ...
    if True:
        ...
        model,
        os.path.join(
        "/nfs/team361/am74/mintflow/revision/skin_perturbation/mintflow_runs/run1_tiger",
