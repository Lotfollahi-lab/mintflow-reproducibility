...
coeff_cutoff_for_rid_reg=0,logistic_coef_cutoff=0):
    ...
    for radius in inputRadius:
        ...
        if flag==1:
            pc_of_sp_clusterid,PCA_of_sc_cluster_accordingto_spatial_clusterid,save_scFactors,save_spFactors, ADDED_dict_clidsp_to_stdnorm_and_W, ADDED_dict_CTindex_to_clidsp = compute_PC_space(input,full_ad_sc)
        ...
        inputdata['ADDED_dict_clidsp_to_stdnorm_and_W'] = ADDED_dict_clidsp_to_stdnorm_and_W
        inputdata['ADDED_dict_CTindex_to_clidsp'] = ADDED_dict_CTindex_to_clidsp
...
def find_PC_of_invidualCluster_in_SC(seed,spatial_integration_modality,scbarcode,iNMFmode,scadata,no_of_pc,spbarcode,spadata, sct_ad_sc_full,celltype_name,cutoff_to_count_exp_cell_population):
    ...
    # print("   before: scadata.X.shape = {}".format(scadata.X.shape))
    sct_ad_sc=scadata[index,:].copy()  # ak:note, as if here gene subsampling happens, but it doesn't happen. and both anndata object are of the same shape as the original `adata` :0
    # print("      after: sct_ad_sc.X.shape = {}".format(sct_ad_sc.X.shape))
    # ak:note, the `index` seems to subsample based on the "correct" cell type label population.
    #   And all of the sudden the "cell-wise" subsampling of the NMF factor looks correct, but there's still some gene subsampling going on.
    ...
    std1 = np.where(std1 < 1e-6, 1.0, std1)
    std2 = np.where(std2 < 1e-6, 1.0, std2)

    ADDED_pergene_stdnorm = std1 + 0.0


    # ind=np.where((std1>0)&(std2>0))
    # index=ind[0]
    # n=len(index)
    # print(">>> n = {}".format(n))  # ak:notes, here is where some genes are subsampled.

    v1=msc + 0.0  # np.zeros((msc.shape[0],n),dtype=float)
    v2=msp + 0.0  # np.zeros((msp.shape[0],n),dtype=float)
    
    v1 = v1/np.expand_dims(std1, 0)
    v2 = v2/np.expand_dims(std2, 0)
    ...
    if spatial_integration_modality=='single':
        ...
        spW=W
        spH=H
        print(celltype_name,"alpha, H size, W size, spH size:", alpha,H.shape,W.shape,spH.shape, "   v1.shape:", v1.shape)
        print("\n\n\n")
    ...
    return transfer_sp_com, transfer_sc_com, sc_spearman,sc_cosine,sc_genenames, H, spH,sc_cluster_mean_exp,sc_cluster_exp_more_than_threshold,alpha, ADDED_pergene_stdnorm, ADDED_Wtoret
...
def compute_PC_space(input,sct_ad_sc_full):
    ...
    ADDED_dict_clidsp_to_stdnorm_and_W = {}
    ADDED_dict_CTindex_to_clidsp = {}
    if flag==1:
        n=len(input.spatialcell_unique_clustername)
        pc_of_sp_clusterid={}
        save_scFactors={}
        ...
        for i in range(n):
            ...
            pc_sp,pc_sc,sc_spearman,sc_cosine,sc_genenames,H, spH,sc_cluster_mean_exp,sc_cluster_exp_more_than_threshold,alpha,ADDED_pergene_stdnorm, ADDED_Wtoret=find_PC_of_invidualCluster_in_SC(
                seed,input.spatial_integration_modality,
                scbarcode,input.iNMFmode,input.ad_sc,input.no_of_pc,spbarcode,input.ad_sp, sct_ad_sc_full,input.spatialcell_unique_clustername[i],input.cutoff_to_count_exp_cell_population
            )
            ADDED_dict_clidsp_to_stdnorm_and_W[clid] = [
                ADDED_pergene_stdnorm, ADDED_Wtoret
            ]
            ADDED_dict_CTindex_to_clidsp[i] = clidsp
    ...
    return pc_of_sp_clusterid,PCA_of_sc_cluster_accordingto_spatial_clusterid,save_scFactors,save_spFactors, ADDED_dict_clidsp_to_stdnorm_and_W, ADDED_dict_CTindex_to_clidsp
