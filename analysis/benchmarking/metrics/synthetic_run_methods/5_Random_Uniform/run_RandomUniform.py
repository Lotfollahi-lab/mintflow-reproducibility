...
Xmic_random_uniform.data = np.round(
    Xmic_random_uniform.data * np.random.rand(Xmic_random_uniform.data.shape[0])
)
Xmic_random_uniform.eliminate_zeros()

Xmic_random_uniform = Xmic_random_uniform.toarray()


np_xspl_gt = adata_unnorm.X.toarray() - adata_int.X
dict_output_e = evaluator.eval(
    np_xspl_gt=np_xspl_gt,
    np_xspl_pred=Xmic_random_uniform,
