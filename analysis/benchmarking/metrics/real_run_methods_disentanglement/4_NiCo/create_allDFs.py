...
# settings ===
obsm_key = 'NiCo_Xmic'
fname_adata_in_eachsubdir = 'adata_NiCo_Result.h5ad'
...
def func_make_df_eval(adata_input):
    ...
    for idx_colsel, colsel in enumerate([
        ...
        X_mic_beforescppnormalizetotal = adata_input.obsm[obsm_key][:, colsel]
        if sparse.issparse(X_mic_beforescppnormalizetotal):
            X_mic_beforescppnormalizetotal = X_mic_beforescppnormalizetotal.toarray()
    ...
    if True: #  str_subdirprefix in subdir:
        ...
        if os.path.isfile(
            ...
            try:
                ...
                if False:  #os.path.isfile(fname_pkl_todump):
