# Addressing R3C1 - Beacon (cellcharter V2) - T cells superficial
...
initDir = '/lustre/scratch126/cellgen/lotfollahi/dv8/mint_flow_bench/cellcharter/results_no_center/xenium_old_beacon/'
save_dir = '/nfs/team361/ms83/data/skin/xenium_cellcharter/analysis/'

adata = sc.read_h5ad(initDir + 'cellcharter_no_center.h5ad')
adata

adata_Tcells = adata[adata.obs['broad_celltypes'].str.startswith("T")].copy()
adata = adata_Tcells

# Load the annotations
NICHE_df = pd.read_csv("/nfs/users/nfs_m/ms83/mintflow-revision/benchmarking/spatial_skin_NICHE_NAMES_annotations.csv")

# Create mapping from cell_id to spatial_niche
NICHE_map = dict(zip(NICHE_df['cell_id'], NICHE_df['NICHE_NAMES']))
subtype_map = dict(zip(NICHE_df['cell_id'], NICHE_df['t_cellsubtypes']))

# Apply to other AnnData using cell_id column
adata.obs['NICHE_NAMES'] = adata.obs['cell_id'].map(NICHE_map)
adata.obs['t_cellsubtypes'] = adata.obs['cell_id'].map(subtype_map)

# Check how many matched
matched = adata.obs['t_cellsubtypes'].notna().sum()
print(f"Successfully mapped {matched} out of {adata.n_obs} cells")

print("T cell subtypes distribution:")
print(adata_Tcells.obs['t_cellsubtypes'].value_counts())
...
adata.obs['t_cellsubtypes']=adata.obs['t_cellsubtypes'].astype('category')
adata.uns['t_cellsubtypes_colors'] = [COLORS[cat] for cat in adata.obs['t_cellsubtypes'].cat.categories]
...
def calculate_metrics_for_niche(adata, niche_name, cluster_column):
    ...
    true_niche = (adata.obs['t_cellsubtypes'] == niche_name).values
...
for i in res: 
    for k in knn:
        ...
        cluster_key = f"{i}_{k}_cluster_cellcharter"
        
        sc.pp.neighbors(adata, use_rep="X_cellcharter", n_neighbors=k, key_added=neighbors_key)
        sc.tl.leiden(adata, resolution=i, key_added=cluster_key, neighbors_key=neighbors_key)
        
        # Calculate metrics for T cell superficial niche
        metrics = calculate_metrics_for_niche(adata, 'Superficial', cluster_key)
...
results_df
results_df.to_csv('/nfs/users/nfs_m/ms83/mintflow-revision/benchmarking/cellcharter/analysis/node_removal/beacon_Tcells_superficial_metrics.csv')
...
for idx, metric in enumerate(['recall', 'precision', 'f1', 'split_entropy']):
    ...
    ax.set_title(f'{metric.replace("_", " ").title()} for T cells superficial Recovery')
    ax.set_xlabel('k-neighbors')
    ax.set_ylabel('Resolution')

plt.tight_layout()
plt.savefig('Tcells_superficial_recovery_metrics.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
plt.suptitle('T cells superficial Recovery Metrics Across All Parameter Combinations', 
             fontsize=12, fontweight='bold', y=1.00)
plt.tight_layout()
plt.savefig('Tcells_superficial_recovery_boxplots.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
niches_to_highlight = ["Superficial"]
custom_colors = []
for cat in adata.obs['t_cellsubtypes'].cat.categories:
    if cat in niches_to_highlight:
        custom_colors.append(COLORS[cat])
    else:
        custom_colors.append('#5D5C59')
...
for idx, (_, row) in enumerate(top_params.iterrows()):
    ...
    # Plot
    sc.pl.umap(adata, color="t_cellsubtypes", 
...
plt.tight_layout()
plt.savefig('Tcells_superficial_top_recoveries.pdf', dpi=300, bbox_inches='tight')
plt.show()

adata.write_h5ad(save_dir + 'adata_cellcharter_no_center_spl_clusters_Tcells_superficial.h5ad')
