...
LOCAL_STAGATE = os.path.join(os.path.dirname(__file__), "local_stagate")
if os.path.isdir(LOCAL_STAGATE):
    sys.path.insert(0, LOCAL_STAGATE)
...
RAD_CUTOFFS = {
    "preprocBy_nb1_preprocessing_V2": 30,
...
def main():
    parser = argparse.ArgumentParser(description="Run STAGATE node-removal variant (no self-loop center aggregation).")
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        "--out-root",
        default="/lustre/scratch126/cellgen/lotfollahi/dv8/mint_flow_bench/STAGATE/without_centre/results",
        help="Output root.",
    )
    ...
    for spec in args.datasets:
        ...
        rad_cutoff = args.rad_cutoff if args.rad_cutoff > 0 else RAD_CUTOFFS.get(dataset, 30)
        run_one(
            dataset,
            h5ad_path,
            args.out_root,
