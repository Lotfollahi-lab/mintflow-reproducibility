# Addressing R3C1 - Beacon (cellcharter V2) - T cells dermis
...
for i in res: 
    for k in knn:
        ...
        # Calculate metrics for T cell dermis niche
        metrics = calculate_metrics_for_niche(adata, 'Dermis', cluster_key)
...
results_df
results_df.to_csv('/nfs/users/nfs_m/ms83/mintflow-revision/benchmarking/cellcharter/analysis/node_removal/beacon_Tcells_dermis_metrics.csv')
...
for idx, metric in enumerate(['recall', 'precision', 'f1', 'split_entropy']):
    ...
    ax.set_title(f'{metric.replace("_", " ").title()} for T cells dermis Recovery')
    ax.set_xlabel('k-neighbors')
    ax.set_ylabel('Resolution')

plt.tight_layout()
plt.savefig('Tcells_dermis_recovery_metrics.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
plt.suptitle('T cells dermis Recovery Metrics Across All Parameter Combinations', 
             fontsize=12, fontweight='bold', y=1.00)
plt.tight_layout()
plt.savefig('Tcells_dermis_recovery_boxplots.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('Tcells_dermis_top_recoveries.pdf', dpi=300, bbox_inches='tight')
plt.show()

adata.write_h5ad(save_dir + 'adata_cellcharter_ no_center_spl_clusters_Tcells_dermis.h5ad')
