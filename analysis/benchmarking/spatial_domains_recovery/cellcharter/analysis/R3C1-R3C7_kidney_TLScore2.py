# Addressing R3C1 and R3C7 - Kidney (cellcharter V2) - TLS core 2
...
for i in res: 
    for k in knn:
        ...
        # Calculate metrics for TLS core 2 niche
...
results_df
results_df.to_csv('/nfs/users/nfs_m/ms83/mintflow-revision/benchmarking/cellcharter/analysis/kidney_TLScore2_metrics.csv')
...
for idx, metric in enumerate(['recall', 'precision', 'f1', 'split_entropy']):
    ...
    ax.set_title(f'{metric.replace("_", " ").title()} for TLS core 2 Recovery')
    ax.set_xlabel('k-neighbors')
    ax.set_ylabel('Resolution')

plt.tight_layout()
plt.savefig('TLScore2_recovery_metrics.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
plt.suptitle('TLS core 2 Recovery Metrics Across All Parameter Combinations', 
             fontsize=12, fontweight='bold', y=1.00)
plt.tight_layout()
plt.savefig('TLScore2_recovery_boxplots.pdf', dpi=300, bbox_inches='tight')
plt.show()

adata.write_h5ad(save_dir + 'adata_cellcharter_spl_clusters_TLScore2_V2.h5ad')
...
plt.tight_layout()
plt.savefig('TLScore2_top_recoveries.pdf', dpi=300, bbox_inches='tight')
plt.show()
