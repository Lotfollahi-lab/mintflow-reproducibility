# Addressing R3C1 and R3C7 - Kidney (cellcharter V2) - TLS border
...
initDir = '/nfs/team361/ms83/data/kidney/xenium_cellcharter/'
save_dir = '/nfs/team361/ms83/data/kidney/xenium_cellcharter/analysis/'

adata = sc.read_h5ad(initDir + 'adata_kidney_all_preprocessed_cellcharter_emb.h5ad')
adata
...
for i in res: 
    for k in knn:
        ...
        # Calculate metrics for TLS border niche
        metrics = calculate_metrics_for_niche(adata, 'CD8+ TLS border', cluster_key)
...
results_df
results_df.to_csv('/nfs/users/nfs_m/ms83/mintflow-revision/benchmarking/cellcharter/analysis/kidney_TLSborder_metrics.csv')
...
for idx, metric in enumerate(['recall', 'precision', 'f1', 'split_entropy']):
    ...
    ax.set_title(f'{metric.replace("_", " ").title()} for TLS border Recovery')
    ax.set_xlabel('k-neighbors')
    ax.set_ylabel('Resolution')

plt.tight_layout()
plt.savefig('TLSborder_recovery_metrics.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
plt.suptitle('TLS border Recovery Metrics Across All Parameter Combinations', 
             fontsize=12, fontweight='bold', y=1.00)
plt.tight_layout()
plt.savefig('TLSborder_recovery_boxplots.pdf', dpi=300, bbox_inches='tight')
plt.show()

adata.write_h5ad(save_dir + 'adata_cellcharter_spl_clusters_TLSborder_V2.h5ad')

# Get top 3 parameter combinations by F1 score
top_params = results_df.nlargest(3, 'f1')[['resolution', 'k_neighbors']]

# Create custom color palette
niches_to_highlight = ["CD8+ TLS border"]
custom_colors = []
...
plt.tight_layout()
plt.savefig('TLSborder_top_recoveries.pdf', dpi=300, bbox_inches='tight')
plt.show()
