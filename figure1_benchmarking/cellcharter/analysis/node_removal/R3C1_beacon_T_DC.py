# Addressing R3C1 - Beacon (cellcharter V2) - T_DC Niche 
...
adata = sc.read_h5ad(initDir + 'cellcharter_no_center_scvi.h5ad')
adata

# Load the annotations
spatial_niche_df = pd.read_csv("/nfs/users/nfs_m/ms83/mintflow-revision/benchmarking/spatial_skin_niche_annotations.csv")

# Create mapping from cell_id to spatial_niche
spatial_niche_map = dict(zip(spatial_niche_df['cell_id'], spatial_niche_df['spatial_niche']))

# Apply to other AnnData using cell_id column
adata.obs['spatial_niche'] = adata.obs['cell_id'].map(spatial_niche_map)

# Check how many matched
matched = adata.obs['spatial_niche'].notna().sum()
...
def calculate_metrics_for_niche(adata, niche_name, cluster_column):
    ...
    true_niche = (adata.obs['spatial_niche'] == niche_name).values
...
for i in res: 
    for k in knn:
        ...
        sc.tl.leiden(adata, resolution=i, key_added=cluster_key, neighbors_key=neighbors_key, flavor="igraph")
        
        # Calculate metrics for T_DC niche
        metrics = calculate_metrics_for_niche(adata, 'T_DC', cluster_key)
...
results_df
results_df.to_csv('/nfs/users/nfs_m/ms83/mintflow-revision/benchmarking/cellcharter/analysis/node_removal/beacon_T_DC_metrics.csv')
...
for idx, metric in enumerate(['recall', 'precision', 'f1', 'split_entropy']):
    ...
    ax.set_title(f'{metric.replace("_", " ").title()} for T_DC Niche Recovery')
    ax.set_xlabel('k-neighbors')
    ax.set_ylabel('Resolution')

plt.tight_layout()
plt.savefig('T_DC_recovery_metrics.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('T_DC_recovery_boxplots.pdf', dpi=300, bbox_inches='tight')
plt.show()

adata.write_h5ad(save_dir + 'adata_cellcharter_no_center_spl_clusters_T_DC_V3.h5ad')

# Get top 3 parameter combinations by F1 score
top_params = results_df.nlargest(3, 'f1')[['resolution', 'k_neighbors']]

# Create custom color palette
niches_to_highlight = ["T_DC"]
custom_colors = []
for cat in adata.obs['spatial_niche'].cat.categories:
...
for idx, (_, row) in enumerate(top_params.iterrows()):
    ...
    # Plot
    sc.pl.umap(adata, color="spatial_niche", 
...
plt.tight_layout()
plt.savefig('T_DC_top_recoveries.pdf', dpi=300, bbox_inches='tight')
plt.show()
