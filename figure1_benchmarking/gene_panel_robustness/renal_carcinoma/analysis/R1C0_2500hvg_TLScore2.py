# Addressing R1C0 - Kidney - 2500 hvg - TLS Core 2
...
initDir = '/nfs/team361/ms83/data/kidney/xenium_mintflow/Outputs_nb1_2500genes/adata_output/'
save_dir = '/nfs/team361/ms83/data/kidney/xenium_mintflow/analysis/'

adata = sc.read_h5ad(initDir + 'adata_mintflow_checkpoint_49.h5ad')
adata
...
for i in res: 
    for k in knn:
        ...
        cluster_key = f"{i}_{k}_cluster_2500hvg"
        
        sc.pp.neighbors(adata, use_rep="MintFlow_Xmic", n_neighbors=k, key_added=neighbors_key)
        sc.tl.leiden(adata, resolution=i, key_added=cluster_key, neighbors_key=neighbors_key, flavor="igraph", n_iterations=2)
        
        # Calculate metrics for TLScore2 niche
        metrics = calculate_metrics_for_niche(adata, 'CD8+ TLS core 2', cluster_key)
...
results_df
results_df.to_csv('2500hvg_TLScore2_metrics.csv')
...
for idx, metric in enumerate(['recall', 'precision', 'f1', 'split_entropy']):
    ...
    ax.set_title(f'{metric.replace("_", " ").title()} for TLScore2 Niche Recovery')
    ax.set_xlabel('k-neighbors')
    ax.set_ylabel('Resolution')

plt.tight_layout()
plt.savefig('TLScore2_2500hvg_recovery_metrics.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
plt.suptitle('TLScore2 Niche Recovery Metrics Across All Parameter Combinations', 
             fontsize=12, fontweight='bold', y=1.00)
plt.tight_layout()
plt.savefig('TLScore2_2500hvg_recovery_boxplots.pdf', dpi=300, bbox_inches='tight')
plt.show()

adata.write_h5ad(save_dir + 'adata_mintflow_checkpoint_49_2500hvg_TLScore2_spl_clusters.h5ad')

# Get top 3 parameter combinations by F1 score
top_params = results_df.nlargest(3, 'f1')[['resolution', 'k_neighbors']]

# Create custom color palette
niches_to_highlight = ["CD8+ TLS core 2"]
custom_colors = []
...
plt.tight_layout()
plt.savefig('TLScore2_2500hvg_top_recoveries.pdf', dpi=300, bbox_inches='tight')
plt.show()
