# Addressing R1C0 - Kidney - 2500 hvg - TLS border
...
for i in res: 
    for k in knn:
        ...
        # Calculate metrics for TLSborder niche
...
results_df
results_df.to_csv('2500hvg_TLSborder_metrics.csv')
...
for idx, metric in enumerate(['recall', 'precision', 'f1', 'split_entropy']):
    ...
    ax.set_title(f'{metric.replace("_", " ").title()} for TLSborder Niche Recovery')
    ax.set_xlabel('k-neighbors')
    ax.set_ylabel('Resolution')

plt.tight_layout()
plt.savefig('TLSborder_2500hvg_recovery_metrics.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
plt.suptitle('TLSborder Niche Recovery Metrics Across All Parameter Combinations', 
             fontsize=12, fontweight='bold', y=1.00)
plt.tight_layout()
plt.savefig('TLSborder_2500hvg_recovery_boxplots.pdf', dpi=300, bbox_inches='tight')
plt.show()

adata.write_h5ad(save_dir + 'adata_mintflow_checkpoint_49_2500hvg_spl_clusters.h5ad')
...
plt.tight_layout()
plt.savefig('TLSborder_2500hvg_top_recoveries.pdf', dpi=300, bbox_inches='tight')
plt.show()
